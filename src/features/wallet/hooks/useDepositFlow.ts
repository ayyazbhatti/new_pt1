      ...
      (event: WsInboundEvent) => {
        if (event.type === 'deposit.request.approved') {
          ...
          if (payload.userId === user?.id) {
            ...
            toast.success(
              `Deposit approved (ID: ${payload.requestId.slice(0, 8)}...). Balance updated to $${payload.newBalance.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`,
              { duration: 6000 }
            )
          }
