...
export function SchemeDetailsModal({ layer, onClose }: SchemeDetailsModalProps) {
  return (
    <div className="fixed inset-0 z-50 overflow-hidden flex items-center justify-center p-2 sm:p-4">
      <div className="absolute inset-0 bg-black/50" onClick={onClose} aria-hidden />
      <div className="relative w-full max-w-[calc(100vw-1rem)] sm:max-w-2xl md:max-w-4xl max-h-[95vh] sm:max-h-[90vh] bg-slate-800 border border-slate-700 rounded-lg flex flex-col shadow-2xl overflow-hidden">
        <div className="flex items-center flex-shrink-0 p-3 sm:p-4 border-b border-slate-700">
          <h2 className="text-base sm:text-lg md:text-xl font-bold text-white truncate flex-1 pr-2">
            Scheme Details: {layer.name}
          </h2>
          <button
            type="button"
            onClick={onClose}
          ...
          <section className="bg-slate-700 p-3 sm:p-4 md:p-6 rounded-lg">
            ...
            <div className="space-y-2 text-sm">
              <div>
                ...
                <span className="text-white">{layer.createdAt ? format(new Date(layer.createdAt), 'PPpp') : '—'}</span>
              </div>
              <div>
                <span className="text-slate-400 block mb-1">Last updated</span>
                <span className="text-white">{layer.updatedAt ? format(new Date(layer.updatedAt), 'PPpp') : '—'}</span>
              </div>
            </div>
          </section>
        </div>
