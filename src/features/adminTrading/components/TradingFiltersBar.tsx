...
export function TradingFiltersBar() {
  ...
  // When no user is selected, sync typed text to filters.search (debounced) so one input does both search and user filter
  const debouncedSetSearch = useDebouncedCallback(
    (value: string) => setFilters({ search: value.trim() || undefined }),
    300
  )
  useEffect(() => {
    if (selectedUserId) return
    debouncedSetSearch(userSearch)
  }, [userSearch, selectedUserId, debouncedSetSearch])

  const handleUserSelect = useCallback(
    (userId: string) => {
      setSelectedUserId(userId)
      setFilters({ userId, search: undefined })
      setUserSearch('')
      setUserSearchResults([])
    },
    [setFilters]
  ...
  return (
    <div className="flex flex-wrap items-center gap-3 p-4 border-b border-border bg-surface-1">
      {/* Search / Filter by user - single search on the left */}
      <div className="relative flex-1 min-w-[200px] max-w-[280px]">
        <Input
          placeholder="Search or filter by user..."
          value={userSearch}
