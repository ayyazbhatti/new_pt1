...
use routes::ai_reports::create_ai_reports_router;
...
use services::data_provider_integrations_service::DataProviderIntegrationsService;
use services::order_event_handler::OrderEventHandler;

pub async fn run_mmdps_sync() -> anyhow::Result<services::admin_symbols_service::SyncMmdpsResult> {
    dotenv::dotenv().ok();
    let database_url =
        std::env::var("DATABASE_URL").map_err(|_| anyhow::anyhow!("DATABASE_URL must be set"))?;
    let pool = create_pool(&database_url).await?;
    let service = services::admin_symbols_service::AdminSymbolsService::new(pool);
    Ok(service
        .sync_from_mmdps(true, true, true, true, false)
        .await?)
}
...
pub async fn run() -> anyhow::Result<()> {
    ...
    let dp_integrations = DataProviderIntegrationsService::new(pool.clone());
    match dp_integrations.get().await {
        Ok(cfg) => {
            let cfg = DataProviderIntegrationsService::merge_with_defaults(cfg);
            if let Err(e) = DataProviderIntegrationsService::sync_to_redis(&redis_pool, &cfg).await {
                tracing::warn!(
                    "Data provider integrations Redis sync failed (non-fatal): {}",
                    e
                );
            } else {
                tracing::info!("Data provider integrations synced to Redis from database");
            }
        }
        Err(e) => tracing::warn!(
            "Could not load data provider integrations (run migrations?): {}",
            e
        ),
    }
