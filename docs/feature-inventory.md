...
| AI endpoint rate limits (Redis) | ⚠️ | `ai_chat.rs` `check_rate_limit`; `ai_reports.rs` `check_report_rate_limit`; platform AI settings `rate_limit_per_minute`, `report_rate_limit_per_minute` `admin_settings.rs` | Throttles AI chat and report generation; **not** a global Axum/tower HTTP limiter on all REST routes |
...
| Real-time SL/TP / liquidation user alerts | ✅ | `backend/auth-service/src/lib.rs` NATS `event.position.closed` → `create_sltp_notifications_and_push` / `create_liquidation_notifications_and_push` (`deposits.rs`); Redis `notifications:push` + `notification.push` NATS; `NotificationsPanel.tsx`, `NotificationBell.tsx`, `notificationsStore.ts` | In-app notification stream (kinds `POSITION_SL` / `POSITION_TP` / `POSITION_LIQUIDATED`); not separate Sonner toasts for those kinds |
| Trader cancel pending order | ✅ | `POST /api/orders/:order_id/cancel` and `/v1/orders/:order_id/cancel` `orders.rs` `cancel_order`; `BottomDock.tsx` `cancelOrderApi`; `orders.api.ts` `cancelOrder` | Distinct from admin force-cancel; pending-only guard in `orders.rs` |
| Order margin estimate API | ✅ | `POST /v1/orders/estimate` and `POST /api/orders/estimate` `orders.rs` `estimate_order_margin`; `orders.api.ts` `estimateOrderMargin` | Powers ticket cost breakdown, free-margin slider sizing, leverage display |
| Reconcile pending orders (DB → Redis) | ⚠️ | `POST /api/orders/sync-pending` and `/v1/orders/sync-pending` `orders.rs` `sync_pending_orders` | Replays missing `order:{id}` Redis keys from Postgres pending rows; requires `trading:view`; no dedicated terminal UI found |
| Terminal price snapshot (REST) | ✅ | `GET /v1/terminal/prices` `terminal_prices.rs`; `terminalPrices.api.ts` | Redis-backed bid/ask snapshot when WS ticks unavailable |
| Order placement idempotency | ⚠️ | `orders.rs` `place_order` stores `order:idempotency:{client_key}` in Redis; `apps/order-engine/src/execution.rs` uses `Keys::idempotency` → `idempo:{user_id}:{key}`; `order_handler.rs` uses `idempotency:{key}` | Duplicate-submit protection exists but Redis key formats differ by execution path |
...
| Deposit approved → user WS + toast | ✅ | `deposits.rs` publishes Redis `deposits:approved`; `ws-gateway` `broadcast_deposit_approved` → `deposit.request.approved` to user connections; `useDepositFlow.ts` updates wallet + `pushNotification` + `toast.success` | Real-time path does not require `deposits` channel subscription (targeted per `userId`) |
| Spot/margin/funding “wallet types” | ⚠️ | `wallets.wallet_type` enum (`database/schema.sql`); `calculate_wallet_balance`, `finance.rs`, `withdrawals.rs`, `ledger_service.rs` filter `wallet_type = 'spot'` | Schema supports multiple types; primary balance paths default to **spot**; no separate margin/funding product UX |
...
| Notifications unread count + read state | ⚠️ | `GET /api/notifications` `deposits.rs` returns `unreadCount` and per-row `read`; `notificationsStore.ts` `markRead` / `markAllRead` | **Client-only** mark-read (no `PATCH` route found); reload may re-show unread from DB |
| Terminal email toggles (SL/TP / liquidation) | ✅ | `user_preferences.rs` `enableSlTpEmail` / `enableLiquidationEmail`; `SettingsPanel.tsx` | Persisted in `user_terminal_preferences` JSON (not mobile push) |
| Terminal preferences persistence | ✅ | `user_preferences.rs`, `preferences.api.ts`, `loadTradingPanelState` localStorage; `favouriteSymbolIds` in preferences JSON (`LeftSidebar.tsx`, `TerminalSymbolsPage.tsx`) | Server-backed **favourite symbols** (`favouriteSymbolIds`) plus local chart/ticket state |
...
| Bulk user CSV result download | ✅ | `BulkUserCreation.tsx` builds `Blob` `text/csv` (`bulk_users_results_*.csv`) | Client-side export of bulk-create results |
...
| MMDPS symbol sync | ✅ | `run_mmdps_sync` `lib.rs`; binary `sync_mmdps` (`src/bin/sync_mmdps.rs`, `Cargo.toml` `[[bin]]`); `AdminSymbolsService::sync_from_mmdps` | One-shot CLI + admin-triggered service sync |
...
| Roles user/admin/super_admin/manager | ✅ | Claims checks across routes (e.g. `admin_tags.rs`, `ai_reports.rs`) | **“agent”** product role: `AgentGuard` / `AgentLayout` / `AgentSidebar` exist under `src/shared/**` but are **not imported** in `src/app/router` (agent shell scaffolded, not mounted) |
...
| Leads CSV export | ✅ | `AdminLeadsPage.tsx` (`leads:export` permission); client `Blob` `text/csv` download | In-browser CSV build (no dedicated streaming export API found) |
...
| Feature | Status | Where it lives (key paths) | One-line description |
|--------|--------|---------------------------|----------------------|
| Support chat admin | ✅ | `support_messages` table; user `GET/POST /v1/users/me/chat`, admin `/api/admin/chat/*` `chat.rs`; user message → NATS `chat.support` + `chat.user.{id}` echo; support reply → `chat.user.{id}`; `ws-gateway` routes NATS to WS; `SupportPage.tsx`, `UserSupportPage.tsx`, `chat.api.ts` | End-to-end support thread with real-time fan-out |
| Voiso embedded panel | ✅ | `/admin/voiso` `AdminVoisoPage.tsx`; build arg `VITE_VOISO_PANEL_URL` | iframe embed |
| Click-to-call / PSTN | ✅ | `POST /api/admin/voiso/click2call` `admin_voiso.rs` (proxies Voiso Click2Call / voice APIs; API key server-side); `docs/voiso-integration-guide.md` | Server-side Voiso integration path present; call quality still depends on Voiso cluster config |
...
| Idempotency keys (Redis) | ⚠️ | `crates/redis-model/src/keys.rs` `idempo:{user_id}:{key}`; `apps/order-engine/src/execution.rs` uses it; `order_handler.rs` uses `idempotency:{key}`; auth `orders.rs` uses `order:idempotency:{key}` | Duplicate protection implemented; **not a single shared key namespace** across all paths |
...
## 18.5. UX / THEMING (TERMINAL & SETTINGS)

| Feature | Status | Where it lives (key paths) | One-line description |
|--------|--------|---------------------------|----------------------|
| Terminal header clock (UTC offset) | ✅ | `RightTradingPanel.tsx` `getTimezoneOffset` + `setInterval` 1s | Displays `UTC±N` from **browser local timezone** (not a saved user preference on server) |
| “Light Theme” control (terminal sidebar) | ⚠️ | `LeftSidebar.tsx` + `themeStore.ts` + `index.html` boot script + `tailwind.config.js` / `globals.css` semantic vars | Toggle persists via Zustand; flips `<html>` `dark` class; terminal core styled for both modes; admin/user panels need light-mode polish |
...
| `agentNavItems` empty | `src/app/config/nav.ts`; `AgentGuard` / `AgentLayout` exist but are **not imported** in `src/app/router/**` — agent shell unused. |
| **Admin Settings “General” save** | `SettingsPage.tsx` “Save Changes” for site name / timezone / currency has **no save handler** wired (local `useState` only). |
...
## Verified Absent (from follow-up audit)

The following reviewer checklist items were **searched for** (grep / file reads) and **no product implementation** was found in this repository:

- **End-user i18n / localization framework** — no `react-i18next` / `i18next` / locale JSON bundles under `src/` (hardcoded English UI strings).
- **Native mobile push** — no `device_token`, FCM, APNs, or Expo integration (distinct from in-app `notification.push` / WebSocket).
- **Per-symbol trading hours / session columns** — `symbols` table (`database/schema.sql`) has `trading_enabled` / `close_only` / `allow_new_orders` but no `trading_hours` / `session_open` / `session_close`; order-engine has no market-hours validator grep match.
- **Failed-login counter / account lockout fields** — `auth_service.rs` `login` records mismatch but does not increment `failed_login_count` or set `locked_until` (no such columns referenced in login path).
- **Global REST API rate limiting middleware** — no `tower_governor` / blanket Axum limit layer on `auth-service` router; throttling is **scoped** to AI routes (see Section 1 row).
- **Admin finance “export transactions” CSV/XLSX** — `finance.api.ts` lists JSON only; no server or client CSV export for the transactions grid (unlike leads + bulk users).
...
|------|----------------|------------------------|
| **Order-engine idempotency canonical path** | Both `execution.rs` (`idempo:{user_id}:{key}`) and `order_handler.rs` (`idempotency:{key}`) define checks; routing depends on which consumer handles `cmd.order.place` in a given deployment. | Trace single production entrypoint for place commands; align keys or document primary path. |
| **Auth REST behind reverse-proxy rate limits** | No nginx/traefik configs in repo for this app’s auth tier. | Confirm infra outside repo (CDN/WAF, API gateway). |
| **`symbols.mock.ts` still imported** | Called out in Section 20; not re-traced in this pass. | Grep imports from admin trading feature. |
| **CI location** | Executive summary already notes CI may live outside repo. | Link to external pipeline or add config to repo. |
